...
fn connection_string() -> String {
    ...
    std::env::var("PYROBOREPLAY_TEST_S3_URL").unwrap_or_else(|_| {
        format!(
            "s3://{}?endpoint=http://localhost:9000&path_style=true",
            bucket
        )
    })
}
...
fn connected_backend() -> S3Backend {
    ...
    backend.connect().expect(
        "failed to connect to local test MinIO (is the container running? see module docs)",
    );
    backend
}
...
fn test_store_and_retrieve_events_and_report_then_delete_mission() {
    ...
    match backend.retrieve_event(&mission_id, "evt-2") {
        Err(StorageError::NotFound(_)) => {}
        other => panic!(
            "expected event to be deleted along with mission, got {:?}",
            other
        ),
    }
    match backend.retrieve_report(&mission_id) {
        Err(StorageError::NotFound(_)) => {}
        other => panic!(
            "expected report to be deleted along with mission, got {:?}",
            other
        ),
    }
    match backend.retrieve_mission(&mission_id) {
...
fn test_list_missions_respects_limit() {
    ...
    for i in 0..3 {
        ...
        backend
            .store_mission(&mission_id, "{}")
            .expect("store_mission failed");
    }

    let all = backend
        .list_missions(None)
        .expect("list_missions(None) failed");
    let matching: Vec<_> = all.iter().filter(|m| m.starts_with(&prefix)).collect();
    assert_eq!(matching.len(), 3);

    let limited = backend
        .list_missions(Some(1))
        .expect("list_missions(Some(1)) failed");
...
fn test_close_disconnects_backend() {
    ...
    backend
        .close()
        .expect("close should succeed while connected");
