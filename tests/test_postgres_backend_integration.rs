...
fn connection_string() -> String {
    std::env::var("PYROBOREPLAY_TEST_POSTGRES_URL").unwrap_or_else(|_| {
        "postgresql://postgres:test@localhost:5433/pyroboreplay_test".to_string()
    })
}
...
fn connected_backend() -> PostgresBackend {
    ...
    backend.connect().expect(
        "failed to connect to local test PostgreSQL (is the container running? see module docs)",
    );
    backend
}
