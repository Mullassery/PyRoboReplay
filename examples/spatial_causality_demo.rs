use pyroboreplay::core::{causality::CausalLink, SpatialCausalityAnalyzer, SpatialContext};
...
fn main() {
    ...
    println!(
        "  Event 2: Position (2.2, 0.0) | Very close to obstacle (0.3m) | Traversability: 40%"
    );
    ...
    if let Some(spatial_link) = analyzer.analyze_spatial_causality(&link1) {
        ...
        println!(
            "  Spatial Impact: {:.0}%",
            spatial_link.spatial_impact * 100.0
        );
        println!(
    ...
    let link2 = CausalLink::new(
        2,
        3,
        "navigation_decision_caused_motion".to_string(),
        0.85,
        300,
    );
