"""MCP Connector for PyRoboReplay - Robot Replay & Analysis"""
...
class ReplayAnalyzer:
    """Robot execution replay analysis with causality"""

    def __init__(self):
        self.mcp_connector: Optional[Any] = None

    def start_mcp_connector(self, port: int = 8777) -> str:
        from pyroboreplay._mcp_tools import PyRoboReplayMCPHandler, PyRoboReplayMCPTools
        self.mcp_connector = _MCPReplayConnector(replay=self, port=port)
...
class _MCPReplayConnector(BaseMCPConnector):
    def __init__(self, replay: ReplayAnalyzer, port: int = 8777):
        super().__init__("PyRoboReplay", port=port)
        self.replay = replay

    def get_mcp_tools(self) -> Dict[str, Any]:
        from pyroboreplay._mcp_tools import PyRoboReplayMCPTools
        return PyRoboReplayMCPTools.get_tools()

    def get_tool_handlers(self) -> Any:
        from pyroboreplay._mcp_tools import PyRoboReplayMCPHandler
        return PyRoboReplayMCPHandler(self.replay)
